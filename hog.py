...
def max_scoring_num_rolls(nums_iter, dice=six_sided):
    ...
    10
    """
    best_score, roll_no = 0, 0
    average = make_averaged(roll_dice, nums_iter)
    for i in range(1,11):
        score = average(i, dice)
        print('{} dice scores {} on average'.format(i, score))
        if score > best_score:
            best_score = score
            roll_no = i
    return roll_no
